...
def compute_saliency_maps(X, y, model):
    ...
    rs = model(X)
    y = rs.gather(1, y.view(-1, 1)).squeeze().to(torch.long)
    loss  = F.cross_entropy(rs, y)
    loss.backward()
    saliency = torch.amax(X.grad.abs(), dim=(1))
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    scores = model(img)
    target_score = scores[0, target_y] - l2_reg*img.norm()
    target_score.backward()
